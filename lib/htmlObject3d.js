...
export default class HtmlObject3D extends THREE.Object3D {
    ...
    constructor(htmlDoc, fonts, labelProps) {
        ...
        this.makeLinesFromWords(blockObj, 0)
        this.printBlocks(blockObj)

        /* blocksArray.forEach(block => {
            this.makeLinesFromWords2(block, labelProps.style.width - 40)
        }) */
    ...
    addWordsDescToBlock(text, style, block) {
        ...
        if (!preFormated) {
            ...
            wordsArr.forEach(word => {
                let spacesNum = word.length - word.trimLeft().length
                let spaceBeforeWidth = spacesNum * fontSize * 0.635
    ...
    */
    ...
    makeLinesFromWords(block, blockY) {
        block.pos.y = blockY
        ...
        if (block.wordDescsArr.length) {
            ...
            block.wordDescsArr.forEach((wordDesc, index) => {
                ...
                if (wordDesc.style['white-space'] !== 'pre') {
                    ...
                    wordY -= wordDesc.height * 1.2
                    wordDesc.pos = { x: wordX + wordDesc.spaceBeforeWidth, y: wordY }
                }
            })
            wordY -= lineHeight
        ...
        block.subBlocksArr.forEach((subBlock) => {
            blockY = this.makeLinesFromWords(subBlock, blockY)
        })
        return blockY
    }
    ...
    printBlocks(block) {
        ...
        else block.subBlocksArr.forEach(subBlock => this.printBlocks(subBlock))

        //return wordY

    }

    makeLinesFromWords3(block) {
        ...
        if (block.wordDescsArr.length) {
            ...
            block.subBlocksArr.forEach((subBlock) => {
                this.makeLinesFromWords(subBlock)
            })
        }
    }

    makeLinesFromWords2(block, maxWidth) {
        const blokStyle = block.style
        const paddingLeft = blokStyle['padding-left'] ? blokStyle['padding-left'] : 0
        const paddingRight = blokStyle['padding-right'] ? blokStyle['padding-right'] : 0

...
export default class HtmlObject3D extends THREE.Object3D {
    ...
    constructor(htmlDoc, fonts, labelProps) {
        ...
        let mergedStyle = Object.assign({}, defaultStyle, labelProps.style);


        let blocksArray = this.walkTheDom(htmlDoc, mergedStyle)

        blocksArray.forEach(block => {
            this.makeLinesFormWords2(block, mergedStyle.width - 40)
        })
        ...
        this.translateX(- mergedStyle.width / 2)
        this.translateY(height / 2)


        if (labelProps.background.color) this.addBackground(mergedStyle.width, height, defaultStyle['font-size'] / 2, labelProps.background)
        this.updateMatrix()
    ...
    walkTheDom(htmlDoc, defaultStyle) {
                ...
                if (node.nodeType === 1) { // Is it an Element node?
                    ...
                    if (newLineEls.includes(elementStyle.display)) {
                        ...
                        let newBlockObj = {
                            ...
                            direction: 'vertical',
                            subBlocksArr: [],
                            wordDescsArr: [] // temporary storage, before we make them into lines
                        }

                        blockObj.subBlocksArr.push(newBlockObj)

                        processChildNodes(node, newBlockObj, mergedStyle)
                    }

                    else processChildNodes(node, blockObj, mergedStyle)
        ...
        this.makeLinesFormWords(blockObj, 560)
    ...
    makeLinesFormWords(block, maxWidth) {
        ...
        if (block.wordDescsArr.length) {
            ...
            block.wordDescsArr.forEach((wordDesc) => {
                wordDesc.pos.x = lineWidthUntilNow
                lineWidthUntilNow += wordDesc.spaceBeforeWidth + wordDesc.width
                // This word won't fit
                if (lineWidthUntilNow > width) {
                    // add the line to the subBlocksArr
                    block.subBlocksArr.push(line)
                ...
                // The word does fit. Add it to the line.
                line.subBlocksArr.push(wordDesc)
            })
            if (line.subBlocksArr.length) block.subBlocksArr.push(line)

            let lineY = 0
            block.subBlocksArr.forEach(line => {
                line.subBlocksArr.forEach(word => {
                    if (word.height * 1.4 > line.height) line.height = word.height * 1.4 // multiply by default line-height spacing
                    line.width += word.spaceBeforeWidth + word.width
                })
                line.pos.y = lineY
                lineY -= line.lineHeight
                block.height += line.lineHeight
            })
            delete block.wordDescsArr
        }
        else {
            const getColumCount = tableBlock => {
                let highestCellCount = 0
                tableBlock.subBlocksArr.forEach( rowBlock => {
                    let cellCount = 0
                    if( rowBlock.style.display === 'table-row') {
                        rowBlock.subBlocksArr.forEach( cellBlock => { 
            ...
            if( block.style.display === 'table') {
                ...
                block.subBlocksArr.forEach( rowBlock => {
                    let cellCount = 0
                    if( rowBlock.style.display === 'table-row') {
                        rowBlock.subBlocksArr.forEach( (cellBlock, index) => { 
                            if( cellBlock.style.display === 'table-cell') { 
                                cellBlock.pos.x = maxWidth / columnCount * index 
                                this.makeLinesFormWords(cellBlock, maxWidth / columnCount)
                            }
                        })
                    }
                })
            }
            else block.subBlocksArr.forEach((subBlock) => {
                this.makeLinesFormWords(subBlock, maxWidth)
            })
        }
    }

    makeLinesFormWords2(block, maxWidth) {
        const blokStyle = block.style
        const paddingLeft = blokStyle['padding-left'] ? blokStyle['padding-left'] : 0
        const paddingRight = blokStyle['padding-right'] ? blokStyle['padding-right'] : 0
        const width = maxWidth - paddingLeft - paddingRight

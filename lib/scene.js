...
export default class Scene {
    ...
    async getHTMLGeometry(url) {
        ...
        const processChildNodes = (parentNode, parentStyle, parentBlockGeom, lineMesh) => {
            ...
            parentNode.childNodes.forEach(node => {
                ...
                if (node.nodeType === 1) { // Is it an Element node?
                    ...
                    let elementStyle, mergedStyle, attrStyle = {}
                    ...
                    switch (node.nodeName) {
                        case 'A':
                            ...
                            mergedStyle = Object.assign({}, parentStyle, elementStyle, attrStyle);
                            return processChildNodes(node, mergedStyle, parentBlockGeom, lineMesh)
                            break;
                        ...
                        case 'DIV':
                            ...
                            mergedStyle = Object.assign({}, parentStyle, elementStyle, attrStyle);
                            addNewBlock( node, mergedStyle, parentBlockGeom )
                            break;
                        ...
                        default:
                            ...
                            processChildNodes(node, parentStyle)
                            break;
                    }
                }
                /* if (node.nodeType === 2) { // Is it a Attr node?
                ...
                if (node.nodeType === 3) { // Is it a Text node?
                    ...
                    if (text.length > 0) { // Does it have non white-space text content?
                        // process text
                        console.log(text, parentStyle)
                        let newlineMesh = printText(text, parentStyle, parentBlockGeom, lineMesh)
                        return newlineMesh // these words will be added when we know we're done with the block
                    }
                }
            })
        }

        const printText = (text, style, parentBlockGeom, lineMesh) => {

            const words = text.split(' ');
            const lineMeshWidth = lineMesh.geometry.boundingBox.max.x
            ...
            for (let n = 0; n < words.length; n++) {
                ...
                let testMesh = new THREE.Mesh(testGeometry, textMaterial)

                if (lineMeshWidth + testWidth > width) {  // test line is too long 

                    // add meshNotProcessedYet to lineMesh
                    meshNotProcessedYet.translate(lineMeshWidth, 0, 0) // position it on the next line
                    lineMesh.merge(meshNotProcessedYet)

                    // get the height of the new line
                    lineMesh.geometry.computeBoundingBox();
                    let lineHeight = lineMesh.geometry.boundingBox.max.y
                    // TODO add line spacing
                    lineHeight += 10

                    // add lineMesh to parentBlockGeom
                    lineMesh.translate(0, -lineHeight, 0) // position it on the next line
                    parentBlockGeom.merge(lineMesh)
                    ...
                    wordsNotProcessedYet = testLine;
                    meshNotProcessedYet = testMesh;
                }
            }
            return meshNotProcessedYet // these words will be added when we know we're done with the block

        }

        const addNewBlock = ( node, mergedStyle, parentBlockGeom ) => {

            console.log('BLOCK')

            let newBlockGeom = new THREE.Geometry();
            let newLineMesh = new THREE.Mesh()
            newLineMesh.geometry.computeBoundingBox();

            let returnedLineMesh = processChildNodes(node, mergedStyle, newBlockGeom, newLineMesh)

            // get the height of the new line
            returnedLineMesh.geometry.computeBoundingBox();
            let lineHeight = returnedLineMesh.geometry.boundingBox.max.y
            // TODO add line spacing
            lineHeight += 10

            // add lineMesh to newBlockGeom
            returnedLineMesh.translate(0, -lineHeight, 0) // position it on the next line
            newBlockGeom.merge(returnedLineMesh)


            // get the height of the new block
            newBlockGeom.geometry.computeBoundingBox();
            let blockHeight = newBlockGeom.geometry.boundingBox.max.y
            // TODO add margin
            blockHeight += 10

            // add newBlockGeom to parentBlockGeom
            newBlockGeom.translate(0, -blockHeight, 0) // position it on the next line
            parentBlockGeom.merge(newBlockGeom)
        ...
        processChildNodes(htmlDoc.body, defaultStyle, htmlGeom, new THREE.Mesh())
        htmlGeom.center()
        return htmlGeom
    }
}
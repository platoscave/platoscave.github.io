...
export default class Scene {
    ...
    //(htmlDoc, regularFont, boldFont, monoFont, 600, 20, calloutBackgroundMat, 'bottomLeft')
    getLabelObj3d(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size, backgroundMat, borderMat, callout) {

        //let labelObj3d = new THREE.Object3D();

        //let linesGeom = await this.makeTextLinesGeom(text, fontName, maxWidth, size)
        const textMat = new THREE.MeshBasicMaterial({ color: 0x404040 });
        let labelObj3d = this.getHTMLGeometry(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size, backgroundMat, borderMat, callout)
        //labelObj3d.add(new THREE.Mesh(linesGeom))
        return labelObj3d
    ...
    getHTMLGeometry(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size, backgroundMat, borderMat, callout) {
            ...
            parentNode.childNodes.forEach(node => {
                ...
                if (node.nodeType === 3) { // Is it a Text node?
                    ...
                    if (text.length > 0) { // Does it have non white-space text content?
                        ...
                        addWordsDescToBlock(text, parentStyle, currentBlock)
                    }
                }
            })
        }

        const addWordsDescToBlock = (text, style, block) => {
            ...
            wordsArr.forEach(word => {
                ...
                // make an exception for the punctuation marks
                if (!!word.match(/^[.,:!?]/)) spaceBeforeWidth = 0
        ...
        const makeLinesFormWords = (block, width) => {
            let line = {
                wordsArr: [],
                lineHeight: 0
            }
            ...
            block.wordDescsArr.forEach((wordDesc, index) => {
                if (lineWidthUntilNow === 0) wordDesc.spaceBeforeWidth = 0
                ...
                line.lineHeight = wordDesc.height > line.lineHeight ? wordDesc.height : line.lineHeight
        ...
        const addLineSegments = (text, style) => {
            ...
            // We must find the line width until now so we can add it durring measurement
            let lineWidthUntilNow = 0
            currentLine.segmentsArr.forEach(segment => lineWidthUntilNow += segment.segmentWidth)
            ...
            for (let n = 0; n < words.length; n++) {
                ...
                if (lineWidthUntilNow + testWidth + fontSize * 0.635 > width) {  // test line is too long 

                    // Add wordsNotProcessedYet to currentLine segmentsArr. This will be the last segment on currentLine
                    if (wordsNotProcessedYet.trim()) { // if we're retuning from a child node, ther may not be a wnpy
                        let segmentDesc = {
        ...
        const printLines = (htmlObj3d) => {
            ...
            blocksArray.forEach((block) => {
                ...
                block.linesArr.forEach((line, lineNum) => {

                    let segmentX = paddingLeft

                    line.segmentsArr.forEach((lineSegment, segNum) => {

                        let text = lineSegment.text
                        let style = lineSegment.style
                        let segmentWidth = lineSegment.segmentWidth
                        ...
                        let textGeometry = new THREE.ShapeGeometry(shapes);
                        ...
                        textGeometry.translate(segmentX, lineY, 0)
                        let mesh = new THREE.Mesh(textGeometry, textMaterial)
                        mesh.updateMatrix();
                        htmlObj3d.add(mesh)

                        //this.mergeMeshes( [htmlObj3d, mesh] ) // cant get this to work

                        // Add bullet or number if we have a list style type
                        if (lineNum === 0 && segNum === 0 && listStyleType) {
                            let geometry
                            if (listStyleType === 'disc') {
                                geometry = new THREE.CircleGeometry(3, 32);
                                geometry.translate(segmentX - 20, lineY + 5, 0)
                            }
                            else {
                        ...
                        if (style['text-decoration'] === 'underline') {
                            let geometry = new THREE.PlaneGeometry(segmentWidth, 2, 2);
                            geometry.translate(segmentX + segmentWidth / 2, lineY + -2, 1)
                            let mesh = new THREE.Mesh(geometry, textMaterial);
                            htmlObj3d.add(mesh)
                        }
                        ...
                        if (style['text-decoration'] === 'line-through') {
                            let geometry = new THREE.PlaneGeometry(segmentWidth, 2, 2);
                            geometry.translate(segmentX + segmentWidth / 2, lineY + fontSize * 0.4, 1)
                            let mesh = new THREE.Mesh(geometry, textMaterial);
                            htmlObj3d.add(mesh)
                        }
                        ...
                        if (style['background-color']) {
                            ...
                            let shape = this.getRoundedRectShape(segmentWidth + 3, height + 3, 2)
                            let geometry = new THREE.ShapeGeometry(shape);
                            geometry.translate(segmentX + segmentWidth / 2, lineY + 4, -1)
                            let mesh = new THREE.Mesh(geometry, textMaterial);
                            htmlObj3d.add(mesh)
                        }

                        segmentX += segmentWidth + 5 // add space
                    })

                    lineY -= line.lineHeight * 1.2 // multiply by default line-height spacing
        ...
        let htmlObj3d = new THREE.Object3D();
        ...
        blocksArray.forEach( block => {
            makeLinesFormWords( block , maxWidth - 40) 
        })
        console.log('blocksArray ', blocksArray)

        printLines(htmlObj3d)
        console.log('htmlGeom ', htmlObj3d)

        htmlObj3d.position.set(- maxWidth / 2, 0, -2)



        let roundedRectShape = this.getRoundedRectShape(maxWidth, 1000, size / 2, callout)

        if (backgroundMat) {

            let geometry = new THREE.ShapeGeometry(roundedRectShape);
            geometry.translate(maxWidth / 2 - 20, 0, -2)
            htmlObj3d.add(new THREE.Mesh(geometry, backgroundMat))
            //linesGeom.computeBoundingBox();
            //boundingBox = geometry.boundingBox
        }
        if (borderMat) {
            ...
            var points = roundedRectShape.getPoints();
            var geometryPoints = new THREE.BufferGeometry().setFromPoints(points);
            geometryPoints.translate(maxWidth / 2 - 20, 0, -2)
            htmlObj3d.add(new THREE.Line(geometryPoints, borderMat))
            //geometryPoints.computeBoundingBox();
            //boundingBox = geometryPoints.boundingBox
        }

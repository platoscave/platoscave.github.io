...
export default class Scene {
    ...
    async getHTMLGeometry(url) {
        ...
        const processChildNodes = (parentNode, parentStyle) => {
            ...
            parentNode.childNodes.forEach(node => {
                ...
                if (node.nodeType === 1) { // Is it an Element node?
                    ...
                    let newLineEls = ['block', 'table', 'list-item', 'table-row']
                    if ( newLineEls.includes(elementStyle.display) ) {
                        let blockDesc = {
                            tag: node.nodeName,
                            lineHeight: 0,
                            style: mergedStyle,
                            lineSegmentsArr: [],//remove
                            linesArr: [],
                        }
                        linesArray.push( blockDesc )
                    }
        ...
        const addLineSegments = ( text, style ) => {
            ...
            let lineNum = linesArray.length - 1
            let currentLine = linesArray[ lineNum ]
            const segmentWidthUntilNow = currentLine.lineSegmentsArr //????
            ...
            for (let n = 0; n < words.length; n++) {
                ...
                if (segmentWidthUntilNow + testWidth > width) {  // test line is too long 
                    ...
                    // add wordsNotProcessedYet to lineMesh
                    let textDesc = {
                        text: wordsNotProcessedYet.trim(),
                        style: style,
                        segmentWidth: widthNotProcessedYet
                    }
                    currentLine.lineSegmentsArr.push(textDesc)
        ...
        const printLines = (htmlGeom) => {
            ...
            linesArray.forEach( (line, index => {
                ...
                let paddingLeft = lineStyle['padding-left'] ? lineStyle['padding-left'] : 0
                let marginTop = lineStyle['margin-top'] ? lineStyle['margin-top'] : 0
                let marginBottom = lineStyle['margin-bottom'] ? lineStyle['margin-bottom'] : 0

                let segmentX = paddingLeft
                lineY -= marginTop

                line.lineSegmentsArr.forEach( (lineSegment) => {
                ...
                lineY -= line.lineHeight + marginBottom
            })
        }

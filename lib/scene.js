...
export default class Scene {
    ...
    getLabelMesh(text, font, maxWidth, size, textMat, backgroundMat, borderMat) {
        ...
        let width = linesGeom.boundingBox.max.x + size
        let height = linesGeom.boundingBox.max.y + size
        let roundedRectShape = this.getRoundedRectShape(width * 2, height * 2, size)
    ...
    makeTextLinesGeom(text, font, maxWidth, size) {
        ...
        let linesArr = []
        //let textHeight = lineHeight + 8 + 20 // add margins
        let textWidth, lineHeight, curWidth
        ...
        for (let n = 0; n < words.length; n++) {
            ...
            if (testWidth > maxWidth) {  // test line is too long, use wordsNotProcessedYet as new line
                ...
                textHeight += lineHeight; // bump line height until now
    ...
    makeChestahedronGeom() {
        ...
        return chestahedronGeom.center()
    }

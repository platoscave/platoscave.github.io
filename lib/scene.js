...
export default class Scene {
    ...
    getLabelObj3d(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size, backgroundMat, borderMat, callout) {
        ...
        let linesGeom = this.getHTMLGeometry(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size)
    ...
    getHTMLGeometry(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size) {
        ...
        const addLineSegments = (text, style) => {
            ...
            currentLine.segmentsArr.forEach( segment => {
                lineWidthUntilNow += segment.segmentWidth
            })
            ...
            if(currentBlock.tag === 'P') debugger
            for (let n = 0; n < words.length; n++) {
                ...
                if (lineWidthUntilNow + testWidth > width) {  // test line is too long 
        ...
        const printLines = (htmlGeom) => {
            ...
            blocksArray.forEach((block) => {
                ...
                lineY -= marginBottom
                if (listStyleType === 'decimal') orderedListNum++
            })
        }

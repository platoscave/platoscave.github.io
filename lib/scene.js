...
export default class Scene {
    ...
    getHTMLGeometry(htmlDoc, regularFont, boldFont, monoFont, maxWidth, size, backgroundMat, borderMat, callout) {
        ...
        const makeLinesFormWords = (block, maxWidth) => {
        ...
        let height = - printBlocks(htmlObj3d) // returns lineY
        console.log('htmlGeom ', htmlObj3d)

        htmlObj3d.position.set(- maxWidth / 2, height / 2, 0)



        let roundedRectShape = this.getRoundedRectShape( maxWidth, height, size / 2, callout)
        ...
        if (borderMat) {
            ...
            var points = roundedRectShape.getPoints();
            var geometryPoints = new THREE.BufferGeometry().setFromPoints(points);
            geometryPoints.translate(maxWidth / 2 - 20, - height / 2, -2)

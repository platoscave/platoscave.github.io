...
export default class HtmlBlock {
    ...
    constructor(text, fonts, style) {
        ...
        this.height = style['margin-left'] ? style['margin-left'] : 0
        this.marginTop = style['margin-left'] ? style['margin-left'] : 0
        this.marginBottom = style['margin-right'] ? style['margin-right'] : 0
    ...
    makeLinesFromWords() {
        ...
        let wordX = this.paddingLeft, wordY = -this.paddingTop
        let lineHeight = 0, lineStartIndex = 0
        //if (this.tag === 'PRE') debugger
        let maxTextWidth = this.width - this.paddingLeft - this.paddingRight
        ...
        this.textBlocksArr.forEach((wordDesc, index) => {
            ...
            if (wordX > maxTextWidth) {
                ...
                for (let i = lineStartIndex; i < index; i++) {
                    this.textBlocksArr[i].pos.y = wordY
                }
            ...
            wordX += wordDesc.maxTextWidth

        })
        wordY -= lineHeight
        for (let i = lineStartIndex; i < this.textBlocksArr.length; i++) {
            this.textBlocksArr[i].pos.y = -this.textBlocksArr[i].maxY * 1.2
        }


        this.height = this.paddingBottom - wordY
    }


    printBlocks(blockX, blockY, object3d) {
        let lineY = 0

...
export default class HtmlBlock {
    ...
    applyWordWrap() {
        ...
        let lineHeight = 0, lineLengthUntilNow = lineStartX
        ...
        this.textBlocksArr.forEach((wordDesc, index) => {
            ...
            lineLengthUntilNow += wordDesc.width + spaceWidth
            ...
            if (index + 1 < this.textBlocksArr.length) { // we still have a next word
                ...
                let testLineLength = lineLengthUntilNow + nextWordDesc.width + spaceWidth
        ...
        if (this.textAlign === 'justify') {
            linesArr.forEach((line, index) => {
                ...
                let space = (maxTextWidth - minWidth) / line.length
                lineLengthUntilNow = lineStartX
                // set pos.x
                line.forEach(wordDesc => {
                    wordDesc.pos.x = lineLengthUntilNow
                    lineLengthUntilNow += wordDesc.width + space
                })
            })
        }
        ...
        if (this.textAlign === 'center') {
            linesArr.forEach(line => {
                ...
                let minWidth = 0
                line.forEach(wordDesc => {
                    minWidth += wordDesc.width + spaceWidth
                })
                minWidth -= spaceWidth // remove last space
                // determin space to distribute
    ...
    printBlock(object3d, orderedListNum) {
